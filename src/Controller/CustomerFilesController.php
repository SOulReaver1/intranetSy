...
class CustomerFilesController extends AbstractController
{
    ...
    public function __construct(
        ...
        SmsAutoRepository $smsAutoRepository,
        // SendSms $sendSms,
        ClientStatutDocumentRepository $clientStatutDocumentRepository,
    ...
    public function index(
        Request $request,
        GlobalStatut $global,
        CustomerFilesStatutRepository $customerFilesStatutRepository,
        CustomerFilesRepository $customerFilesRepository,
        ...
        foreach (
            $customerFilesRepository->getUniqueDepartment($this->globalStatut)
            as $value
        ) {
        ...
        foreach (
            $customerFilesRepository->getUniqueMetreur($this->globalStatut)
            as $key => $value
        ) {
            $metreurList[$key]['email'] = $value['email'];
            $metreurList[$key]['id'] = $value['id'];
        }
        ...
        if ($editStatutForm->isSubmitted() && $editStatutForm->isValid()) {
            ...
            if ($data['customer_files']) {
                foreach ($customer_files_ids as $value) {
                    $customerFile = $customerFilesRepository->find(
                        intval($value)
                    );
                ...
                $this->addFlash(
                    'success',
                    count($customer_files_ids) .
                        ' fiche(s) on(t) bien été(s) modifiée(s) !'
                );
            } else {
        ...
        if ($request->isMethod('get')) {
            ...
            if ($request->query->get('departments')) {
                $this->session->set(
                    'departments',
                    $request->query->get('departments')
                );
            }
            if ($request->query->get('metreurs')) {
                $this->session->set(
                    'metreurs',
                    $request->query->get('metreurs')
                );
            }

            if (
                !$this->findByRoles->findByRole('ROLE_ADMIN', $this->getUser())
            ) {
                ...
                if (count($user_global_statuts) > 0) {
                    if (
                        in_array(
                            $this->globalStatut,
                            $user_global_statuts->toArray()
                        )
                    ) {
                        $this->session->set(
                            'global',
                            $this->globalStatut->getId()
                        );
                    } else {
                        $this->addFlash(
                            'error',
                            "Vous n'avez pas accès au statut global : " .
                                $this->globalStatut->getName()
                        );
        ...
        $table = $dataTableFactory
                ...
                'render' => function ($value, $context) {
                    $show = sprintf(
                        ...
                        $this->generateUrl('customer_files_show', [
                            'global' => $this->globalStatut->getId(),
                            'id' => $value,
                        ])
                    );
                    $edit = sprintf(
                        '<a href="%s" class="btn btn-primary">Modifier</a>',
                        $this->generateUrl('customer_files_edit', [
                            'global' => $this->globalStatut->getId(),
                            'id' => $value,
                        ])
                    );
            ...
            ->createAdapter(ORMAdapter::class, [
                ...
                'query' => function (QueryBuilder $builder) {
                    ...
                    if (
                        $this->session->get('departments') &&
                        $this->session->get('statut') &&
                        $this->session->get('metreurs')
                    ) {
                        return $builder
                            ->select('c, customer_statut')
                            ->andWhere('c.global_statut = :g')
                            ->andWhere('c.department IN(:departments)')
                            ->andWhere('c.metreur IN(:metreurs)')
                            ->andWhere('customer_statut.id = :statut')
                            ->setParameter('g', $this->globalStatut)
                            ->setParameter(
                                'departments',
                                explode(',', $this->session->get('departments'))
                            )
                            ->setParameter(
                                'metreurs',
                                explode(',', $this->session->get('metreurs'))
                            )
                            ->setParameter(
                                'statut',
                                $this->session->get('statut')
                    ...
                    if (
                        ...
                        return $builder
                            ->select('c, customer_statut')
                            ->andWhere('c.global_statut = :g')
                            ->andWhere('c.department IN(:metreurs)')
                            ->andWhere('customer_statut.id = :statut')
                            ->setParameter('g', $this->globalStatut)
                            ->setParameter(
                                'metreurs',
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($customerFile->getInstaller()) {
                ...
                $notificationService->sendNotification(
                    [$customerFile->getInstaller()],
                    'Une nouvelle fiche vous à été attribuer !',
                    '/' . $customerFile->getId()
                );
                // // Send mail
                $mailer->sendMail(
                    [$customerFile->getInstaller()],
                    'Nouveau ticket Lergon\'Home',
                    'customer_files/email_template/installer.html.twig',
    ...
    public function show(
        ...
        if (
            !in_array('ROLE_INSTALLATEUR', $this->getUser()->getRoles()) ||
            $customerFile->getInstaller() === $this->getUser()
        ) {
     ...
     */
    public function editGlobalStatut(
        Request $request,
        GlobalStatut $global,
        CustomerFiles $customerFile
    ) {
        ...
        $form = $this->createForm(
            UpdateCustomerGlobalStatutType::class,
            $customerFile
        );
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash(
                'success',
                'Le statut global de la fiche à bien été modifier !'
            );
            return $this->redirectToRoute('customer_files_index', [
                'global' => $this->session->get('global'),
            ]);
    ...
    public function edit(
        ...
        CustomerFiles $customerFile,
        ProviderRepository $provider,
        CustomerFilesRepository $repository
    ): Response {
        // if($customerFile->getGlobalStatut() === $global){
        $form = $this->createForm(
            UpdateCustomerFileType::class,
            $customerFile,
            ['global' => $global]
        );
        $form->handleRequest($request);
        ...
        $password = $this->createForm(
            UpdateCustomerPasswordType::class,
            $customerFile
        );
        ...
        if ($form->isSubmitted() && $form->isValid()) {
        ...
        } elseif ($password->isSubmitted() && $password->isValid()) {
            $this->getDoctrine()
                ->getManager()
                ->flush();
            ...
            $this->addFlash(
                'success',
                'Le mot de passe de la fiche a bien été modifié !'
            );
            // $this->smsAutoRepository->checkSms($customerFile);
            return $this->redirectToRoute('customer_files_show', [
                'id' => $customerFile->getId(),
        ...
        } elseif ($mail->isSubmitted() && $mail->isValid()) {
            $this->getDoctrine()
                ->getManager()
                ->flush();
            ...
            $this->addFlash(
                'success',
                'L\'email de la fiche a bien été modifié !'
            );
            // $this->smsAutoRepository->checkSms($customerFile);
            return $this->redirectToRoute('customer_files_show', [
                'id' => $customerFile->getId(),
        ...
        return $this->render('customer_files/edit.html.twig', [
            ...
            'providers' => $provider->findAll(),
            'params' => $repository->getProviderParams(
                $customerFile->getProduct()
                    ? $customerFile->getProduct()->getProvider()
                    : null
            ),
        ]);
        // }
